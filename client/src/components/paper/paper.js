...
function paperReducer(state, action) {
    switch (action.type) {
        ...
        case 'uploadFile': {
            ...
            if (!action.file || action.file.type !== 'application/pdf') return {
                ...state,
                uploadedFile: null,
            };
            return {
...
const Paper = () => {
    ...
    useEffect(() => {
        const loadFiles = async () => {
            ...
            for (let paper of papers) {
                promises.push(fetch(`/api/papers/files/${paper.file._id}`)
                .then(res => res.blob())
                .then(blob => {
    ...
    }, [state.isExtracting, state.extractedFile, state.inputName, state.selectedGroup])
    ...
    const dataTable = state.papers.filter(paperFilter).map(paper => {
        return <PaperTableRow paper={paper} dispatch={dispatch} key={paper._id}/>
    }).reverse();
    const uploadText = state.uploadedFile ? state.uploadedFile.name : 'No file selected';

    return (
        <div className='Paper'>
            <div className={state.windowOpened ? 'paper-layout' : 'paper-layout disabled'}
            onClick={e => dispatch({ type: 'closeWindow' })}></div>
            <ExtractWindow active={state.windowOpened} file={state.extractedFile}
            groups={state.groups}
            selectedGroup={state.selectedGroup}
            databases={state.databases} inputValue={state.inputName}
            windowMessage={state.windowMessage}
            selected={state.selected} dispatch={dispatch}/>
            <div className='w3-light-grey'>
                <div className={`w3-container w3-blue w3-center ${state.progress ? 
                '' : 'disabled'}`} style={{
                    width: state.progress || 0 + '%'
                }}>{state.progress || 0}%</div>
            </div>
            <div className="paper-header">
                <div className="search-label">Search by file name: </div>
                <input className="search-paper" value={state.searchName}
                onChange={e => dispatch({
                    type: 'searchName',
                    value: e.target.value,
                })}></input>
                <input type="file" id="paper-file" className="upload-input" 
                onChange={e => dispatch({
                    type: 'uploadFile',
                    file: e.target.files[0] 
                })}></input>
                <label htmlFor="paper-file" className="upload-label">Choose a file</label>
                <button className={`upload-file-btn ${state.uploadedFile ? '' : 'disabled'}`}
                onClick={e => dispatch({
                    type: 'saveFile',
                })}>Upload</button>
                <div className={`upload-file-name ${state.uploadedFile ?
                'activated' : ''}`}>{uploadText}</div>
                <button className={`remove-uploaded ${state.uploadedFile ?
                '' : 'disabled'}`} onClick={e => dispatch({
                    type: 'closeUploaded',
                })}>x</button>
            </div>
