...
const Paper = () => {
    ...
    useEffect(() => {
        const loadFiles = async () => {
            ...
            const files = res.data.response;
            const promises = [];
            for (let file of files) {
                promises.push(fetch(`/api/papers/files/${file.filename}`).then(res => res.blob())
                .then(blob => {
    ...
    const dataTable = state.files.filter(fileFilter).map(file => {
