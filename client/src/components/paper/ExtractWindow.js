...
const ExtractWindow = (props) => {
    if (!props.active) {
        return (
            <div className="paper-window disabled"></div>
        )
    }
    ...
    groupOptions.push({ key: props.groups.length, text: 'Personal', value: 'personal' })
    const options = props.databases.filter(database => {
    ...
    options.push({ key: props.databases.length, text: 'New database', value: 'new' });
    ...
    return (
        <div className="paper-window">
            ...
            <div className="select-group-btn">
                <select className="select-group" placeholder="Select a group"
                value={props.selectedGroup}
                onChange={(e) => props.dispatch({
                    type: 'selectGroup',
                    value: e.target.value,
                })}>
                    {groupOptions.map(option => <option value={option.value} key={option.key}>
                        {option.text}
                    </option>)}
                </select>
            </div>
            ...
            <div className="select-database-btn">
                <select className="select-database" placeholder="Select a database"
            ...
            <input className="name-input" id="database-name" autoComplete="off"
            readOnly={props.selected && props.selected !== 'new'}
            value={props.inputValue} onChange={e => props.dispatch({
                type: 'inputName',
                value: e.target.value,
            })}></input>
            ...
            <button className="paper-extract" onClick={e => props.dispatch({
                type: 'extractBibtex',
            })}>Extract</button>
        </div>
    )
