...
const GroupBox = (props) => {
    ...
    return (
        <div className={`group-box ${props.isSelected ? 'selected-group' : ''}`}
        onClick={e => {
            if (!e.target.classList.contains('box-click-btn')) props.dispatch({
            type: 'selectGroup',
            group: props.group,
        })}}>
            ...
            onClick={e => { if (!isRequested) props.dispatch({
                type: 'clickGroup',
                group: props.group,
            })}}>{btnName}</button>
        </div>
