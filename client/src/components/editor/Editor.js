    ...
    switch (action.type) {
        ...
        case 'saveEntry': {
            for (let tag of entryFields[state.entryChanges.entryType].required) {
                ...
                state.entryChanges.entryTags[tag] === '') return {
                    ...state,
                    errorMessage: `Entry field '${tag}' cannot be empty`,
                };
            }
            if (state.entryChanges.citationKey === '') return {
                ...state,
...
const Editor = () => {
    ...
    useEffect(() => {
        const createFile = async () => {
            if (state.selectedGroup === 'user') {
                ...
                const err = res.data.error ? res.data.error.reason : null;
                dispatch({ type: 'finishCreating', ok: res.data.ok, 
                file: res.data.response, err })
            } else {
        ...
        const uploadFile = async () => {
            ...
            fileReader.onload = async () => {
                const res = await utils.uploadDatabase(fileReader.result, state.inputName,
                    state.selectedGroup);
                const err = res.data.error ? res.data.error.reason : null;
                dispatch({ type: 'finishUploading', ok: res.data.ok, 
                file: res.data.response, err });
            }
            fileReader.onerror = () => {
    ...
    useEffect(() => {
        const changeName = async () => {
            ...
            const err = res.data.error ? res.data.error.reason : null;
            dispatch({ type: 'finishSaving', ok: res.data.ok, 
            file: res.data.response, err })
        }
    ...
    useEffect(() => {
        const createEntry = async () => {
            ...
            const err = res.data.error ? res.data.error.reason : null;
            dispatch({ type: 'finishCreatingEntry', ok: res.data.ok, 
            entry: res.data.response, err })
        }
    ...
    useEffect(() => {
        const saveEntry = async () => {
            ...
            const err = res.data.error ? res.data.error.reason : null;
            dispatch({ type: 'finishSavingEntry', ok: res.data.ok,
            entry: res.data.response, err })
        }
    ...
    return (
        <div className="Editor">
            ...
            <div className="editor-error-message" style={{
