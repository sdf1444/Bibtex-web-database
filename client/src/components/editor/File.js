...
const File = (props) => {
    const info = props.file.user ? { label: 'User', name: props.file.user.username }
    : { label: 'Group', name: props.file.group.name }
    const d = new Date(props.file.updatedAt);
    return (
        <div className={props.isSelected ? 'file-box chosen-file' : 'file-box'}
        onClick={e => props.dispatch({
            ...
            <div className="file-head">
                ...
                <img className={`file-icon ${!props.isAllowed ? '' : 'disabled'}`} 
