...
const EntryList = (props) => {
    ...
    if (!props.active || !props.file) return (
        <div className="entry-div"></div>
    )
    
    const searchParams = parseSearch(props.search);
    const entryList = (Object.keys(searchParams.entryTags).length !== 0 
    || searchParams.entryType) ? props.file.entries.filter(entry => {
        let val = true;
        if (searchParams.entryType) {
            val = val && (
                entry.entryType.toLowerCase().startsWith(searchParams.entryType)
        ...
        if (values.length !== 0) {
            ...
            for (let [key, value] of values) {
                if (key === 'citationKey') {
                    if (entry[key].startsWith(value) || entry[key].includes(value)) {
                        matches = true;
                    }
                    continue;
                }
                if (entry.entryTags[key] && (entry.entryTags[key].startsWith(value)
    ...
    const listDivs = entryList.map(entry => {
        const isChosen = props.selectedEntry && props.selectedEntry._id === entry._id
       return <Entry isChosen={isChosen} isNew={false} 
       entry={entry} key={entry._id} onClick={() => props.dispatch({
           type: 'selectEntry',
           entry,
       })}/>
    })
    ...
    const newEntryDiv = (
        <Entry isChosen={!!props.newEntry} isNew={true} 
        isDisabled={!props.newEntry} entry={props.newEntry} 
        key="newEntry" onClick={() => props.dispatch({ type: 'unselectNew' })}/>
    )
    return (
        <div className="entry-div">
            <Menu compact className="add-entry-btn">
                <Dropdown text="Add entry" options={options} 
                simple item onChange={(e, data) => {if (props.isSelectedAllowed) props.dispatch({
                    type: 'selectNew',
                    value: data.value,
                })}}></Dropdown>
            </Menu>
            <div className="entry-search">
                ...
                <input className="entry-search-input" id="entry-search"
