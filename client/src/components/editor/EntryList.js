...
const EntryList = (props) => {
  ...
  const handleAddEntryClick = (e, data) => {
    ...
    props.selectEntry({
      entry: { new: true, value: data.value },
      database: database,
    });
  };
  const unselectNew = () => {
  ...
  const newClassname =
    state && state.new ? 'entry-new chosen-entry' : 'entry-new disabled';
  const newText = state ? `New ${state.value}` : '';
  let newEntryDiv = (
    <button className={newClassname} onClick={unselectNew}>
      {newText}
    </button>
  );
  return (
    <div className='entry-div'>
      ...
      <a
        ...
        download={`${database.bibtexdatabasename}.bib`}
      >
        Download
      </a>
    </div>
