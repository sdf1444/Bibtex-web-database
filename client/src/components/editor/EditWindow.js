...
const EditWindow = (props) => {
    ...
    entryFields[props.entry.entryType].extra.forEach(tag => infoList.push(
        <div className="edit-info edit-info-extra" key={tag}>
            <div className="info-label" data-tag={tag}>{tag}:</div>
            <textarea className="info-input"
            spellCheck="false"
            value={props.entryChanges.entryTags[tag] || ''}
            onChange={e => props.dispatch({ 
            ...
            <div className="edit-window-main">
                <form className="edit-window-form">
                    <div className="edit-window-head">
                        Editing entry: @{props.entry.entryType.toLowerCase()}</div>
                    ...
                    <div className="edit-window-footer">
                        <button className="edit-cancel-btn" type="button"
                        onClick={e => props.dispatch({
                            type: 'closeWindow',
                        })}>Cancel</button>
                        <button className="edit-save-btn" type="submit"
                        onClick={e => {
                            e.preventDefault();
                            props.dispatch({
                                type: 'saveEntry',
