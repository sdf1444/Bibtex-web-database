...
const FileList = (props) => {
    ...
    const listDivs = props.fileList.map(file => {
        const className = (props.selectedFile && props.selectedFile._id === file._id) ? 
        'chosen-file' : '';
        const fileName = file.bibtexdatabasename > 15 ?
        file.bibtexdatabasename.slice(0, 18) + '...' : file.bibtexdatabasename;
        return (
            <button key={file._id} id={file._id}
            className={className} onClick={e => props.dispatch({
                type: 'select',
                file: file,
            })}>
                {fileName}
