...
router.get('/', auth, async (req, res, next) => {
    ...
    }).populate('owner').populate('users')
    .populate('joinRequests').exec();
    res.data = groups;
    return next();
})
...
router.get('/me', auth, async (req, res, next) => {
    const groups = await Group.find({ users: req.user.id }).populate('owner')
    .populate('users').populate('joinRequests').exec();
    res.data = groups;
    return next();
})
...
router.get('/own', auth, async (req, res, next) => {
    const groups = await Group.find({ owner: req.user.id }).populate('owner')
    .populate('users').populate('joinRequests').exec();
    res.data = groups;
    return next();
})
...
router.get('/withme', auth, async (req, res, next) => {
    ...
    }).populate('owner').populate('users').populate('joinRequests').exec();
    res.data = groups;
    return next();
})
...
router.post('/join-request', auth, async (req, res, next) => {
    ...
    if (group.users.map(user => user.toString()).includes(req.user.id.toString())) {
        res.data = { err: 'This user is already in a group' }
        return next();
    }
    if (group.joinRequests.map(user => user.toString()).includes(req.user.id.toString())) {
...
router.post('/accept', auth, async (req, res, next) => {
    ...
    if (!group.joinRequests.map(user => user.toString()).includes(req.body.userId.toString())) {
    ...
    try {
        const newGroup = await group.save();
        res.data = await newGroup.populate('owner').populate('users')
        .populate('joinRequests').execPopulate();
        return next();
    } catch (err) {
        res.data = { err: err.message }
        return next();
...
router.post('/ban', auth, async (req, res, next) => {
    ...
    if (!group.users.map(user => user.toString()).includes(req.body.userId.toString())) {
        res.data = { err: 'This user is not in group' }
        return next();
    }
    group.users = group.users.filter(user => {
